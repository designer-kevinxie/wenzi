...
- **正文不要複述引文。** 讀者已經在圖上讀完了,再抄一遍等於什麼都沒說,
  還會因為圖是繁體、文案是簡體而顯得不協調。
  寫圖上沒有的東西:這句話的出處背景、作者當時的處境、譯本的來歷、
  或是它為什麼值得停下來看一眼。
- 但作者名與書名必須自然出現在正文裡 —— 平台搜尋只索引文字,
  圖片裡的字讀不到,正文是唯一的搜尋入口。
  需要引述時只取三五個字的關鍵詞,不要整句照抄。
- 禁止這類沒有資訊量的填充句:「這句話讓我思考」「你是否也曾」
  「或許真正的 X 是」「在這個快節奏的時代」。
  一句話如果換成任何別的引文都成立,就刪掉。寧可短。
